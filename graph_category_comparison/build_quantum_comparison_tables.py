...
RESULTS_ROOT = "quantumSolver_results"
OUT_ROOT = "quantum_comparison"
...
def load_results(folder):
    ...
    for fname in os.listdir(folder):
        ...
        with open(os.path.join(folder, fname)) as f:
            data = json.load(f)
        if "tour_cost" in data and "tour" in data:
...
print(f"[✓] Saved quantum comparison table → {out_path}")
...
for rname, graphs in graphs_by_range.items():
    ...
    print(f"[✓] Saved quantum cascade table → {out_path}")

print("\nAll quantum comparison tables generated successfully.")
