...
RESULTS_ROOT = "exactSolver_results"
OUT_ROOT = "exact_comparison"
...
# Cascade results: graph -> factor -> data
...
for graph, full_data in full_results.items():
    ...
    row = {
        ...
        "full_cost": full_data["optimal_cost"],
        "full_runtime": full_data["runtime_sec"],

        "edgegnn_cost": edgegnn_results[graph]["optimal_cost"],
        "edgegnn_runtime": edgegnn_results[graph]["runtime_sec"],

        "cascade_best_cost": best_data["optimal_cost"],
...
comparison_path = os.path.join(OUT_ROOT, "comparison_table.csv")
...
for rname, graphs in graphs_by_range.items():
    ...
    for graph in graphs:
        ...
        for factor in factors:
            if factor in cascade_results[graph]:
                ...
                row[f"{factor}_cost"] = data["optimal_cost"]
    ...
    print(f"[✓] Saved cascade factor table → {out_path}")

print("\nAll exact comparison tables generated successfully.")
