...
# # Paths
# # ------------------------
# root_dir = os.getcwd()  # run this script in project root
# full_dir = os.path.join(root_dir, "graphss_chosen")       # full TSP instances
# pruned_dir = os.path.join(root_dir, "prunedd_graphs/stage1")  # pruned
...
# # Loop over instances
# # ------------------------
# for instance_name in os.listdir(pruned_dir):
#     instance_folder = os.path.join(pruned_dir, instance_name)
#     if not os.path.isdir(instance_folder):
#         continue

#     # ---- PRUNED .par ----
#     sparse_tsp_path = os.path.join(instance_folder, f"{instance_name}_sparse.tsp")
#     if not os.path.exists(sparse_tsp_path):
#         print(f"No sparse .tsp for {instance_name}, skipping")
#         continue

#     sol_file_pruned = os.path.join(results_dir, f"{instance_name}_pruned.sol")
#     par_file_pruned = os.path.join(params_dir, f"{instance_name}_pruned.par")
#     #MAX_CANDIDATES = min(50, n)
#     with open(par_file_pruned, "w") as f:
#         f.write(f"PROBLEM_FILE = {sparse_tsp_path}\n")
#         f.write(f"OUTPUT_TOUR_FILE = {sol_file_pruned}\n")
#         f.write("RUNS = 1\n")
#         f.write(f"MAX_CANDIDATES = {15}\n")
#         f.write("MOVE_TYPE = 5\n")
#     print(f"Created PRUNED .par file for {instance_name}")

#     # ---- FULL .par ----
#     full_tsp_path = os.path.join(full_dir, f"{instance_name}.tsp")
#     if not os.path.exists(full_tsp_path):
#         print(f"No full .tsp for {instance_name}, skipping full .par")
#         continue

#     sol_file_full = os.path.join(results_dir, f"{instance_name}_full.sol")
#     par_file_full = os.path.join(params_dir, f"{instance_name}_full.par")
...
#         f.write("MOVE_TYPE = 5\n")
#         f.write("PRECISION = 1\n")
#     print(f"Created FULL .par file for {instance_name}")

# print("Done. Parameter files are ready in 'parameters/'")
...
# K values to sweep
k_values = [2, 3, 4, 5, 6, 7, 8, 10, 12, 15, 20, 25]
